...
ALTER TABLE off_line_dws.dws_shop_deal_info
    ADD COLUMN shop_name String AFTER shop_id;

ALTER TABLE off_line_dws.dws_shop_deal_info
    DROP PARTITION '2021-08-04';
ALTER TABLE off_line_dws.dws_shop_deal_info
    ADD COLUMN freight_money String AFTER order_type;
alter table off_line_dws.dws_shop_deal_info
    drop column "shelve_time";
ALTER TABLE dws_shop_sku_info
    DROP PARTITION '2021-08-05';
...
(
    ...
    ENGINE = MergeTree()
        ...
        ORDER BY (shop_id)
        SETTINGS index_granularity = 8192;
ALTER TABLE off_line_dws.dws_shop_sku_info
    DROP PARTITION '2021-08-04';
ALTER TABLE off_line_dws.dws_shop_sku_info
    ADD COLUMN freight_money String AFTER order_type;
ALTER TABLE off_line_dws.dws_shop_sku_info
    ADD COLUMN shelve_time String AFTER cid_name;
drop table off_line_dws.dws_shop_freight_info on cluster bip_ck_cluster;
CREATE TABLE off_line_dws.dws_shop_freight_info on cluster bip_ck_cluster
(
    shop_id                       String,
    order_type                    String,
    sku_id                        String,
    payment_succeed_freight_money Decimal(10, 2),
    payment_freight_money         Decimal(10, 2),
    dt                            String
)
    ENGINE = MergeTree()
        PARTITION BY (dt)
        ORDER BY (shop_id)
        SETTINGS index_granularity = 8192;
-- CREATE TABLE off_line_dws.dws_shop_refund_money on cluster bip_ck_cluster
-- (
--     shop_id                       String,
--     sku_id                        String,
--     refund_type                   String,
...
--
select *
from off_line_dws.dws_shop_freight_info;
...
(
    ...
    refund_income_money  Decimal(10, 2) comment '成功退款佣金',
    ...
    ENGINE = MergeTree()
        ...
        ORDER BY (shop_id)
        SETTINGS index_granularity = 8192;
ALTER TABLE off_line_dws.dws_shop_deal_refund_info
    ADD COLUMN refund_income_money Decimal(10, 2) AFTER all_money
...
drop table off_line_dws.dws_shop_group_user_info on cluster bip_ck_cluster;
...
from (
      select dt,
             payment_money + payment_freight_money                                                             as "总订单金额",
             income_money                                                                                      as "总收入",
             sale_order_number                                                                                 as "总订单数",
             CAST((payment_succeed_money + payment_succeed_freight_money) / buyer_id_number as Decimal(10, 2)) as "客单价",
             0                                                                                                    "退款金额",
             0                                                                                                    "自提点订单金额",
             0                                                                                                    "自提点订单收入",
             0                                                                                                    "自提点订单数量",
             pick_number                                                                                       as "自提点数量"
      from (
               select dt,
                      ...
                      count(distinct buyer_id)                                                     as buyer_id_number,
                      ifNull(count(distinct case when pick_id = '' then null else pick_id end), 0) as pick_number
               from dws_shop_deal_info
               where shop_id = '2000000027'
                 and order_type = 'ALL'
                 and dt between '2021-08-05' and '2021-08-05'
               group by dt
               ) a
               left join (
          select *
          from dws_shop_freight_info
          where dt between '2021-08-05' and '2021-08-05'
            and shop_id = '2000000027'
            and order_type = 'ALL'
          ) b
                         on a.dt = b.dt
      union all
...
from (
      ...
      from dws_shop_deal_info
      where shop_id = '2000000091'
        and order_type = 'ALL'
        and dt between '2021-07-26' and '2021-07-26'
      union all
      select 0                 as "总订单金额",
             0                 as "总收入",
             0                 as "总订单数",
...
from (
         ...
         where shop_id = '2000000027'
           and order_type = 'TB'
           and dt between '2021-08-01' and '2021-08-31'
         group by shop_id
         ) a
         left join (
    select shop_id,
...
from (
     ...
     (select shop_id,
      ...
      where shop_id = '2000000027'
        and dt between '2021-08-01' and '2021-08-01'
      group by shop_id) b
     on a.shop_id = b.shop_id
         all
         full join (
...
select count(distinct case when b.last_time != '' and a.paid = 2 and a.refund = 0 then a.buyer_id end)    as "老成交用户数",
       ...
       sum(case when b.last_time != '' and a.paid = 2 and a.refund = 0 then a.payment_succeed_moneys end) as "老成交用户金额",
       sum(case when b.last_time = '' and a.paid = 2 and a.refund = 0 then a.payment_succeed_moneys end)  as "新成交用户金额"
from (
         select shop_id,
                buyer_id,
                paid,
                refund,
                sum(payment_succeed_money) as payment_succeed_moneys
         from dws_shop_deal_info
         where shop_id = '2000000111'
           and order_type = 'TB'
           and dt between '2021-08-03' and '2021-08-03'
           and buyer_id != ''
         group by shop_id, buyer_id, paid, refund) a
         left join (
    select shop_id, buyer_id, last_time
...
where shop_id = '2000000091'
  and order_type = 'TB'
  and dt between '2021-07-27' and '2021-07-27'
group by buyer_id, buyer_name;
--客户分析-交易构成-TC
select count(distinct case when b.last_time != '' and a.paid = 2 and a.refund = 0 then a.buyer_id end)   as "老成交用户数",
       count(distinct case when b.last_time = '' and a.paid = 2 and a.refund = 0 then a.buyer_id end)    as "新成交用户数",
...
select count(distinct case when a.buyer_id != '' then a.buyer_id end)                               as "总订单用户数",
       ...
       ifNull(count(distinct
                    case when a.paid = 2 and a.refund != 9 and a.refund != 10 and a.buyer_id != '' then a.buyer_id end),
              0)                                                                                    as "成交用户数",
       ifNull(count(distinct case
                                 when b.last_time != '' and a.paid = 2 and a.refund != 9 and a.refund != 10 and
                                      a.buyer_id is not null and
                                      a.buyer_id != ''
                                     then a.buyer_id end), 0)                                       as "成交老用户数",
       ifNull(count(distinct case
                                 when b.last_time = '' and a.paid = 2 and a.refund != 9 and a.refund != 10 and
                                      a.buyer_id is not null and
                                      a.buyer_id != ''
                                     then a.buyer_id end), 0)                                       as "成交新用户数",
       count(distinct case when a.paid = 2 and a.refund != 9 and a.refund != 10 then a.buyer_id end) /
       ...
       ifNull(intDivOrZero(count(distinct case
                                              when b.last_time != '' and a.paid = 2 and a.refund != 9 and
                                                   a.refund != 10 and
                                                   a.buyer_id is not null and
                                                   a.buyer_id != '' then a.buyer_id end),
                           count(distinct case when a.buyer_id != '' then a.buyer_id end)), 0)      as "老成交客户占比",
       CAST(intDivOrZero(sum(payment_succeed_money),
                         count(distinct case when a.buyer_id != '' then a.buyer_id end)) as String) as "客单价"
from (
         select shop_id,
                buyer_id,
                paid,
...
select case
       ...
       case
           ...
           when uv = 0 then '0'
           else concat(CAST(round((CAST(ifNull(userNumber, 0) as Int64) / CAST(ifNull(uv, 0) as Int64)) * 100,
                                  2) as String), '%') end     as "访客-支付转换率"
from (
         select count()                                                                                         pv,
                count(distinct case when loginToken == '' or loginToken is null then ip else loginToken end) as uv,
                day
         from default.dwd_page_log_all
         where shopId = '2000000192'
           and day between '2021-08-03' and '2021-08-03'
         group by day
         ) a
         all
         full join (
    select
--            count(distinct case when paid = 2 and CAST(buyerId as Int64) != 0 then buyerId end) as userNumber,
--            sum(case when paid = 1 then 0 else 1 end)                                           as orderNumber,
--            sum(case when paid = 1 then 0 else (totalMoney - discountMoney) + freightMoney end) as money,
--            day
*
    from (
          select if(buyerShopId != '', buyerShopId, CAST(buyerId as String)) as buyerId,
                 paid,
                 totalMoney,
                 discountMoney,
                 freightMoney,
          ...
          from (
                   ...
                   left join
               (
                   select distinct buyerShopId, orderId
                   from default.dwd_outbound_bill_detail_all
                   where shopId = 2000000192
                     and day between '2021-08-03' and '2021-08-03'
                   ) b
...
-- 单品分析标题头
select sku_pic_url as "商品信息",
       shelve_time as "上架时间",
       cid_name    as "分类名称",
       item_name   as "商品名称"
from dws_shop_sku_info
where shop_id = '2000000111'
  and dt between '2021-08-04' and '2021-08-04'
  and order_type = 'ALL'
  and sku_id = '2000000394';
--     shop_id = #{shop_id} and dt between #{start_time} and #{end_time} and order_type = 'ALL'
...
from (select sku_id,
      ...
      from dws_shop_sku_info
      where shop_id = '2000000027'
        and dt between '2021-08-02' and '2021-08-02'
        and sku_id = '2000000036'
        and order_type = 'ALL'
      group by sku_id, sku_pic_url, cost_price, payment_price, item_name) a
         left join
     (select sku_id,
...
select sku_id,
       `商品信息`,
       cast(`成本价` as String)           as `成本价`,
       cast(`单价` as String)            as `单价`,
       `浏览量`,
       `访客数`,
       `商品名称`,
       `支付件数`,
       `成交的下单用户数`,
       cast(`支付金额` as String)             `支付金额`,
       intDivOrZero(`成交的下单用户数`, `访客数`) as "访客转化率",
       `总下单用户数`
from (select sku_id,
             sku_pic_url                as "商品信息",
...
-- -----------------------交易分析-整体看板-all
select dt,
       count(distinct case when buyer_id != '' then buyer_id end)                        as "下单人数",
       ...
       count(distinct order_id)                                                          as "下单笔数",
       sum(payment_succeed_money)                                                        as "成交金额",
       intDivOrZero(sum(payment_succeed_money),
                    count(distinct case when paid = 2 and refund = 0 then buyer_id end)) as "客单价"
from dws_shop_deal_info
where shop_id = '2000000406'
  and dt between '2021-08-03' and '2021-08-03'
  and order_type = 'TC'
group by dt;
...
select refund_reason                             as "退货原因",
       ...
       intDivOrZero(refund_money, b.all_monery)  as "退款金额比",
       intDivOrZero(refund_number, b.all_number) as "退款笔数比",
       dt
from (
         select dt,
                shop_id,
...
select payment_succeed_number                                 as "支付成功数",
       ...
       refund_money                                           as "成功退款金额",
       refund_number                                          as "成功退款数量",
       round(divide(refund_number, orders_succeed_number), 2) as "退款率",
       dt
from (
         select dt,
                shop_id,
...
select dt                                                  as "时间",
       province_name                                       as "省份",
       cast(payment_buyer_number as String)                as "支付人数",
       cast(payment_succeed_money as String)               as "支付金额",
       divide(payment_succeed_money, total_province_money) as "支付比例"
from (select dt,
             province_name,
             ifNull(count(distinct case when paid = 2 and refund = 0 then buyer_id end), 0) as payment_buyer_number,
             sum(payment_succeed_money)                                                     as payment_succeed_money
      from dws_shop_deal_info
      where shop_id = '2000000091'
        and order_type = 'TB'
     ...
     (select dt,
      ...
      group by dt) b
     on a.dt = b.dt
where a.payment_succeed_money != 0;
-- --------------------------------商品概况
--     商品分析-商品整体概览-商品转换
...
from dws_shop_deal_info
where shop_id = '2000000027'
  and dt between '2021-07-28' and '2021-07-28'
  and order_type = 'ALL';
-- select
--     0 as "加购件数",
--     sum(num) as "下单件数",
...
select b.item_number                                                               as "在架商品数",
       a.sale_number                                                               as "动销商品品种数",
       a.buyer_number                                                              as "下单客户数",
       a.sale_order_number                                                         as "下单笔数",
       a.payment_nums                                                              as "下单商品件数",
       a.payment_succeed_number                                                    as "成交商品件数",
       a.payment_succeed_money                                                     as "成交金额",
       concat(CAST(round(a.buyer_number / c.user_number, 2) * 100 as String), '%') as "商品转换率",
       dt
from (
         select dt,
                sale_number,
                ...
                sale_order_number,
                payment_nums,
                payment_succeed_number,
                payment_succeed_money - refund_money + payment_succeed_freight_money as payment_succeed_money
         from (
                  select dt,
                         count(distinct case when paid = 2 then sku_id end)                      as sale_number,
                         count(distinct case when paid = 2 and buyer_id != '' then buyer_id end) as buyer_number,
                         sum(sale_order_number)                                                  as sale_order_number,
                         sum(num)                                                                as payment_nums,
                         sum(payment_num)                                                        as payment_succeed_number,
                         sum(payment_succeed_money)                                              as payment_succeed_money
                  from dws_shop_deal_info
                  where shop_id = '2000000027'
                    and order_type = 'ALL'
                  ...
                  left join (
             select dt,
                    sum(payment_freight_money)         as payment_freight_money,
                    sum(payment_succeed_freight_money) as payment_succeed_freight_money
             from dws_shop_freight_info
             where shop_id = '2000000027'
               and dt between '2021-08-05' and '2021-08-05'
             group by dt
...
select item_name                                                as "商品名称",
       paid_buyer                                               as "支付人数",
       a.payment_money                                  as "支付金额",
       divide(a.payment_money, b.payment_money) as "支付金额占总支付金额比例"
from (
       select
              *
       from
         ...
         select dt,
                item_name,
                count(distinct buyer_id)   as paid_buyer,
                sum(payment_money) as payment_money
         from dws_shop_deal_info
         where shop_id = '2000000027'
           and dt between '2021-08-05' and '2021-08-05'
...
select sku_code                         as "sku编码",
       item_name                        as "商品名称",
       sum(inbound_num)                 as "库存",
       CAST(sum(total_money) as String) as "金额",
       dt
from dws_shop_inbound_bill_record
where shop_id = '2000000027'
  and dt between '2021-08-01' and '2021-08-01'
...
from (
     ...
     (
         ...
         from (
               ...
               from (
                        select *
                        from default.dwd_order_detail_all
                        where day = cast(toDate(now()) as String)
                          and shopId = 2000000027
                        ) a
                        inner join
                    (select orderId, buyerId
                     from default.dwd_order_all
                     where day = cast(toDate(now()) as String)
                       and shopId = 2000000027
                       and paid = 2
                     group by orderId, buyerId ) b
...
-- 实时概况--商品看板---TOP5支付排行
select itemName as "商品名称", paid_goods_number "支付数量", money_number "支付金额"
from (
      select a.itemName, sum(a.num) as paid_goods_number, sum(paid_money_number) + b.freightMoney as money_number
      from (select orderId,
                   itemId,
                   itemName,
                   num,
            ...
            from default.dwd_order_detail_all
            where day = cast(toDate(now()) as String)
              and shopId = 2000000027) a
               inner join
           (select orderId, freightMoney
            from default.dwd_order_all
            where shopId = 2000000027
              and day = cast(toDate(now()) as String)
              and paid = 2
            group by orderId, freightMoney) b
           on a.orderId = b.orderId
      group by a.itemId, a.itemName, b.freightMoney
         )
...
-- 团购
select *
from dws_shop_group_user_info
where shop_id = '2000000027'
  and dt between '2021-07-26' and '2021-07-26';

select case
           when a.dt = '' and b.dt = '' then c.dt
           when a.dt = '' and c.dt = '' then b.dt
           else a.dt end as dt,
       payment_money,
       income_money,
       sale_order_number,
...
from (select dt,
             ...
             sum(income_money)        as income_money,
             sum(sale_order_number)   as sale_order_number,
             count(distinct buyer_id) as dis_user
      from dws_shop_deal_info
      where shop_id = '2000000423'
        and dt between '2021-07-30' and '2021-07-30'
        and order_type = 'TG'
      group by dt) a
         full join
     (
         select dt,
                ...
                count(case when '2021-07-30' = create_time then user_id end) as ben
         from dws_shop_group_user_info
         where shop_id = '2000000423'
           and dt between '2021-07-30' and '2021-07-30'
         group by dt
...
select count(distinct case when paid = 2 and refund = 0 and buyerId != 0 then buyerId end)         as userNumber,
       ...
       sum(case when paid = 2 and refund = 0 then (totalMoney - discountMoney) + freightMoney end) as money
from default.dwd_order_all final
where groupLeaderShopId = '2000000423'
  and day = cast(toDate(now()) as String)
--     客户分析-地域分布
select dt,
       province_name                                             as "省份",
       payment_buyer_number                                      as "支付人数",
       cast(payment_succeed_money as String)                     as "支付金额",
       intDivOrZero(payment_succeed_money, total_province_money) as "支付比例"
from (select dt,
             province_name,
             count(distinct case when paid = 2 and refund = 0 then buyer_id end) as payment_buyer_number,
             sum(payment_succeed_money)                                          as payment_succeed_money
...
from (
    ...
    select *
    from dws_shop_freight_info
    where dt between '2021-08-04' and '2021-08-04'
      and shop_id = '2000000111'
      and order_type = 'ALL'
    ) b
                   on a.dt = b.dt
